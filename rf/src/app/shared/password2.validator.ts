...
export const PasswordStrengthValidator = function (
  ...
  if (upperCaseCharacters.test(value) === false) {
    return {
      passwordStrength: `text has to continue Upper case characters,current value ${value}`,
    };
  }
  let lowerCaseCharacters = /[a-z]+/g;
  if (lowerCaseCharacters.test(value) === false) {
    return {
      passwordStrength: `text has to continue lower case characters,current value ${value}`,
    };
  }
